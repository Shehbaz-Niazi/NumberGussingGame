...
async function getStarted() {
  ...
  console.log(
    chalk.bgWhite.blue(
      `Your Guess is ${askQuery.userGuess} And System Generated is ${decimal}`
    )
  );
  if (askQuery.userGuess === decimal) {
    ...
    console.log(
      chalk.yellow.bold(
        `Congratulation Your Your Guess is Correct. Your Score is ${score} `
      )
    );
  } else {
    console.log(
      chalk.red.bold(
        `Your Guess is incorrect. Better Luck Next Time your score id ${score} `
      )
    );
  }
}
